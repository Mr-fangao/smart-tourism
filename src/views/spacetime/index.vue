      ...
      <div class="maintools">
        ...
        <div class="content son">
          ...
          <div class="find">
            ...
            <div class="bottom">
              <el-button-group>
                <el-button style="background-color: rgb(0,206,209); color: #FFF; top: 12px" icon="el-icon-search" size="small" @click="btnSelect">搜索</el-button>
                <!-- <el-button type="primary" icon="el-icon-s-order" size="small">统计</el-button> -->
                <el-button style="background-color: rgb(0,206,209); color: #FFF; top: 12px" icon="el-icon-refresh-left" size="small" @click="chongzhi">重置</el-button>
              </el-button-group>
              <div>
...
export default {
  ...
  methods: {
    ...
    tian() {
      ...
      this.word = '冷热点分析是GIS中的一种空间分析方法，是对数据集中的每一个要素计算 Getis-Ord Gi* 统计（称为 G-i-星号）。得到 z 得分和 p 值，对应统计学中的聚类，将数据分为高值和低值，其目的在于突出空间特征，颜色越深表示区域高/低值聚集程度越强。'
    },
    re() {
      this.ceng = 1
      if (this.ceng === 1) {
      ...
      this.word = '通过构建城市地理加权回归模型，分析我国拐卖儿童案件发生与区域教育水平、地区生产总值、人均GDP、人均可支配收入、公路里程数等的关联。'
    },
    ying() {
      this.ceng = 2
      if (this.ceng === 2) {
      ...
      this.word = '通过分析测算空间关联局域指标（Local Indicators of Spatial Association, LISA）显著水平，通过局部自相关分析方法，分析中国拐卖儿童整体空间关联趋势，得出不同尺度范围下我国各区域拐卖之间的关联和影响。'
    },
    jv() {
      this.ceng = 3
      if (this.ceng === 3) {
      ...
      this.word = '新兴时空热点分析是GIS中的一种空间分析方法，能够根据某一段时间内已存在的地理分布数据做后续发展的时间和空间分析，通过新兴时空热点分析，获取被拐儿童热点和冷点趋势，分析我国拐卖儿童犯罪的时空演化特征，包括拐卖儿童案发数随时间变化的整体趋势、整体的范围、空间等级变化的整体趋势以及犯罪重心随时间变化的整体趋势。'
    },
    feng() {
      this.ceng = 4
      if (this.ceng === 4) {
      ...
      this.word = '分别计算 x 和 y 方向上的标准距离，用于定义一个包含所有要素分布的椭圆的轴，用于展示空间要素在空间分布中的趋势，用以反映中国拐卖儿童的空间分布和移动特征。'
    },
    san() {
      this.ceng = 5
      if (this.ceng === 5) {
