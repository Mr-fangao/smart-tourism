<template>
  <div style="position: absolute; width: 100%; height: 93.6%; margin-top: -1px">
    <div id="Querymodule">
      ...
      <div>
        <el-table :data="formdate" style="width: 100%" height="143">
          <el-table-column prop="OBJECTID" label="时间" />
          <el-table-column prop="CenterX" label="中心点x" />
          <el-table-column prop="CenterY" label="中心点y" />
          <el-table-column prop="Rotation" label="旋转角度" />
          <el-table-column prop="XStdDist" label="标准距离长轴" />
          <el-table-column prop="YStdDist" label="标准距离短轴" />
        </el-table>
      </div>
    </div>
      ...
      loadModules(
        ...
        ([
            ...
            document
              ...
              .addEventListener("click", function name(params) {
                ...
                for (let index = 0; index < that.shijian; index++) {
                  ...
                  chaTuoYuan(
                    "editor",
                    datestarts + "-01-01 00:00:00",
                    dateends + "-01-01 00:00:00",
                    color
                  );
                  datestarts++;
                }
          ...
          function chaTuoYuan(params1, i, y, color) {
            // gp服务
            const gpUrl =
              "http://114.98.239.36:6080/arcgis/rest/services/BabyHome/StandardDeviationEllipse/GPServer/StandardDeviationEllipse";
            ...
            gp.submitJob(params).then((jobInfo) => {
              ...
              // 监听执行结束
              gp.waitForJobCompletion(jobInfo.jobId, options).then(
                (jobInfo2) => {
                  for (let index = 0; index < that.shijian; index++) {
                    that.jindu = index;
                  }
            ...
            function maopao(params) {
              ...
              for (let i = params.length - 1; i > 0; i--) {
                ...
                for (let j = 0; j < i; j++) {
                  // 从第一个元素到第i个元素
                  if (
                    params[j].attributes.开始时间 >
                    params[j + 1].attributes.开始时间
                  ) {
                    temp = params[j];
                    params[j] = params[j + 1];
                    params[j + 1] = temp;
            ...
            function drawResultData(result) {
              ...
              var mapUrl =
                "http://114.98.239.36:6080/arcgis/rest/services/ShaanXiDisease/StandardDeviationEllipse/MapServer/jobs/" +
                jobId;
              ...
              queryTask.execute(query).then(function (response) {
                ...
                if (that.graphicall.length === that.shijian) {
                  ...
                  document.getElementById("Querymodule").style.display =
                    "block";
                  ...
                  for (let index = 0; index < that.graphicall.length; index++) {
                    that.lineGraphic.push([
                      that.graphicall[index].geometry.x,
                      that.graphicall[index].geometry.y,
                    ]);
                  }
                  for (
                    let index = 0;
                    index < that.lineGraphic.length - 1;
                    index++
                  ) {
                    const polyline = {
                      type: "polyline", // autocasts as new Polyline()
                      paths: [
                        that.lineGraphic[index],
                        that.lineGraphic[index + 1],
                      ],
                    };
