  ...
  <div id="selectRegion">
    ...
    <div class="eject">
      <div class="nei">
        ...
        <div class="choice">
          <ul>
            <li v-for="(value, index) in sheng" :key="index" class="sheng" @click="shengC($event, value)">{{ value }}</li>
          </ul>
        </div>
        <!-- 市 -->
        <div class="choice">
          <ul>
            <li v-for="(value, index) in shi" :key="index" class="shi" @click="shiC($event, value)">{{ value }}</li>
          </ul>
        </div>
        <!-- 县 -->
        <div class="choice">
          <ul>
            <li v-for="(value, index) in xian" :key="index" class="xian" @click="xianC($event, value)">{{ value }}</li>
          </ul>
        </div>
        <!-- 底 -->
        <div class="choice" @click="confirm()">
          <div id="Scs">
            ...
            <div class="state">
              <i class="el-icon-circle-check" style="position: absolute;top: 52%;transform: translateY(-50%);left: 5px;" /><span> 确认</span>
            </div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    shiC(even, value) {
      ...
      if (that.guo.sub[that.shi.indexOf(value)].sub !== undefined) {
        for (let i = 0; i < that.guo.sub[that.shi.indexOf(value)].sub.length; i++) {
